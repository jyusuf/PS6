...
public class Person_Test {
	...
	public static void setUpBeforeClass() throws Exception {
		...
		Date dDate = null;
		try {
			dDate = new SimpleDateFormat("yyyy-MM-dd").parse("1995-10-18");
		} catch (ParseException e) {
			e.printStackTrace();
		}
		
		per1 = new PersonDomainModel();
		per1.setFirstName("Jama");
		per1.setLastName("Yusuf");
		per1.setBirthday(dDate);
		per1.setCity("Newark");
		per1.setPostalCode(19711);
		per1.setStreet("111 Happy Lane");
	}
